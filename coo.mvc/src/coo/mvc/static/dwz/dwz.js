 ...
 * @author ZhangHuihua@msn.com ---------------------------------------------------------- These functions use the same 'format' strings as the java.text.SimpleDateFormat class, with minor exceptions. The format string consists of the following abbreviations:
 * 
 * Field | Full Form | Short Form -------------+--------------------+----------------------- Year | yyyy (4 digits) | yy (2 digits), y (2 or 4 digits) Month | MMM (name or abbr.)| MM (2 digits), M (1 or 2 digits) | NNN (abbr.) | Day of Month | dd (2 digits) | d (1 or 2 digits) Day of Week | EE
 * (name) | E (abbr) Hour (1-12) | hh (2 digits) | h (1 or 2 digits) Hour (0-23) | HH (2 digits) | H (1 or 2 digits) Hour (0-11) | KK (2 digits) | K (1 or 2 digits) Hour (1-24) | kk (2 digits) | k (1 or 2 digits) Minute | mm (2 digits) | m (1 or 2 digits) Second | ss (2 digits) | s (1 or 2 digits)
 * AM/PM | a |
 * 
 * NOTE THE DIFFERENCE BETWEEN MM and mm! Month=MM, not mm! Examples: "MMM d, y" matches: January 01, 2000 Dec 1, 1900 Nov 20, 00 "M/d/yy" matches: 01/20/00 9/2/00 "MMM dd, yyyy hh:mm:ssa" matches: "January 01, 2000 12:30:45AM" ----------------------------------------------------------
 */
(function() {
	 ...
	 * 
	 * This function takes a date string and a format string. It matches If the date string matches the format string, it returns the date. If it does not match, it returns 0.
	 * 
	 * @param {Object}
	 *            val
	 * @param {Object}
	...
	$.rwdrag = {
		...
		drag: function(e) {
			...
			if(data.options.drag) {
				data.options.drag.apply(current.el, [
						current.el, e
				]);
			}
...
(function($) {
	...
	function completed(cancel) {
		...
		$(this).triggerHandler("change.dwz-accordion", [
			options.data
		], options.change);
	}
...
(function($) {
	$.fn.extend({
		...
		/**
		 * options: reverse[true, false], eventType[click, hover], currentIndex[default index 0] stTab[tabs selector], stTabPanel[tab panel selector] ajaxClass[ajax load], closeClass[close tab]
		 */
