			...
			if(url) {
				if($tab.hasClass("external")) {
					...
					if($.isEmptyObject(op.data)) { // 获取pagerForm参数
						var $pagerForm = $("#pagerForm", $panel);
						op.data = $pagerForm.size() > 0 ? $pagerForm.eq(0).serializeArray() : {};
			...
			function tdHtml(field) {
				...
				switch(field.type) {
					...
					case 'lookup':
						...
						break;
					case 'attach':
						html = '<input type="hidden" name="' + field.lookupGroup + '.' + field.lookupPk + suffix + '"/>' + '<input type="text" name="' + field.name + '" size="' + field.size + '" readonly="readonly" class="' + field.fieldClass + '"/>' + '<a class="btnAttach" href="'
								+ field.lookupUrl + '" lookupGroup="' + field.lookupGroup + '" ' + suffixFrag + ' lookupPk="' + field.lookupPk + '" width="560" height="300" title="查找带回">查找带回</a>';
						break;
					case 'enum':
						$.ajax({
							type: "POST",
		...
		checkboxCtrl: function(parent) {
			return this.each(function() {
				...
				$trigger.click(function() {
					var group = $trigger.attr("group").escape();
