		...
		function initSuffix($tbody) {
			$tbody.find('>tr').each(function(i) {
				$(':input, a.btnLook, a.btnAttach, span', this).each(function() {
					...
					if($this.attr('id'))
						$this.attr('id', $this.attr('id').replaceSuffix(i));
					if($this.attr('customvalid'))
						$this.attr('customvalid', $this.attr('customvalid').replaceSuffix(i));
					if($this.attr('maxDate'))
						$this.attr('maxDate', $this.attr('maxDate').replaceSuffix(i));
					if($this.attr('minDate'))
						$this.attr('minDate', $this.attr('minDate').replaceSuffix(i));
					if($this.attr('minRelation'))
						$this.attr('minRelation', $this.attr('minRelation').replaceSuffix(i));
		...
		function tdHtml(field) {
			...
			switch(field.type) {
				...
				case 'lookup':
					...
					break;
				case 'attach':
					html = '<input type="hidden" name="' + field.lookupGroup + '.' + field.lookupPk + suffix + '"/>' + '<input type="text" name="' + field.name + '" size="' + field.size + '" readonly="readonly" class="' + field.fieldClass + '"/>' + '<a class="btnAttach" href="' + field.lookupUrl
							+ '" lookupGroup="' + field.lookupGroup + '" ' + suggestFrag + ' lookupPk="' + field.lookupPk + '" width="560" height="300" title="查找带回">查找带回</a>';
					break;
				case 'enum':
					$.ajax({
						type: "POST",
