...
public class CaptchaImageGenerator {
	...
	private void setBackgroudColor(Graphics2D graphics) {
		...
		graphics.fillRect(0, 0, captchaImageConfig.getWidth(),
				captchaImageConfig.getHeight());
	}
	...
	private void setNoise(Graphics2D graphics) {
		for (int i = 0; i < captchaImageConfig.getNoise(); i++) {
			...
			int circleRadius = (int) (Math.random()
					* captchaImageConfig.getHeight() / 2.0);
			...
			graphics.drawOval(circleX, circleY, circleRadius * 2,
					circleRadius * 2);
		}
	}
