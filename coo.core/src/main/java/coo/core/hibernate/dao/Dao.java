...
public class Dao<T> {
  ...
  public String getCacheName() {
    if (clazz.isAnnotationPresent(org.hibernate.annotations.Cache.class)) {
      org.hibernate.annotations.Cache cache =
          clazz.getAnnotation(org.hibernate.annotations.Cache.class);
  ...
  public void removeBy(String name, Object value) {
    Query query = createQuery("delete from " + clazz.getName() + " where " + name + "=?", value);
    query.executeUpdate();
  }
  ...
  public Boolean isUnique(T entity, String propNames) {
    ...
    for (String name : nameList) {
      criteria.add(Restrictions.eq(name, BeanUtils.getField(entity, name)));
    }
    // 更新实体类时应该排除自身
  ...
  public Page<T> findPage(String hql, Integer pageNo, Integer pageSize, Integer totalCount,
    ...
    Page<T> page = new Page<>(totalCount, pageNo, pageSize);
    Query query = createQuery(hql, values);

    List<T> list =
        query.setFirstResult((page.getNumber() - 1) * pageSize).setMaxResults(pageSize).list();
  ...
  public Page<T> findPage(Criteria criteria, Integer pageNo, Integer pageSize, Integer totalCount) {
    ...
    Page<T> page = new Page<>(totalCount, pageNo, pageSize);
    List<T> list =
        criteria.setFirstResult((page.getNumber() - 1) * pageSize).setMaxResults(pageSize).list();
    page.setContents(list);
    return page;
  }
   ...
   */
  @SuppressWarnings("unchecked")
  public Iterator<T> searchByLazy(FullTextCriteria criteria, Integer maxResults) {
  ...
  public Page<T> searchPage(FullTextCriteria criteria, SearchModel searchModel) {
    ...
    return searchPage(criteria, searchModel.getPageNo(), searchModel.getPageSize());
  }
   ...
   */
  @SuppressWarnings("unchecked")
  public Page<T> searchPage(FullTextCriteria criteria, Integer pageNo, Integer pageSize) {
    ...
    Page<T> page = new Page<>(total, pageNo, pageSize);
  ...
  public Integer count(Criteria criteria) {
    ...
    Field orderEntriesField = BeanUtils.findField(CriteriaImpl.class, "orderEntries");
    @SuppressWarnings("unchecked")
    List<CriteriaImpl.OrderEntry> orderEntries =
        (List<CriteriaImpl.OrderEntry>) BeanUtils.getField(impl, orderEntriesField);
    BeanUtils.setField(impl, orderEntriesField, new ArrayList<CriteriaImpl.OrderEntry>());

    int totalCount =
        Integer.parseInt(criteria.setProjection(Projections.rowCount()).uniqueResult().toString());
  ...
  public Integer count() {
    Criteria criteria = createCriteria();
    return Integer
        .parseInt(criteria.setProjection(Projections.rowCount()).uniqueResult().toString());
  }
  ...
  private void initSearchFields() {
    ...
    List<Field> embeddedEntityFields = BeanUtils.findField(clazz, IndexedEmbedded.class);
