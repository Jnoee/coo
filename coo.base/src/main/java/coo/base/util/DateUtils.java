...
public class DateUtils {
  ...
  private static final String[] FORMATS = new String[] {DAY, MONTH, MINUTE, SECOND, MILLISECOND,
      DAY_N, MONTH_N, MINUTE_N, SECOND_N, MILLISECOND_N};

  /**
   * 将字符串解析成Date对象。<br>
   * 该方法尝试用[yyyy-MM/yyyy-MM-dd/ yyyy-MM-dd HH:mm/yyyy-MM-dd HH:mm:ss/yyyy-MM-dd HH:mm:ss SSSS/
   * yyyyMM/yyyyMMdd/yyyyMMddHHmm/ yyyyMMddHHmmss/yyyyMMddHHmmssSSSS]格式进行解析，如果无法解析将抛出异常。
   * 
   * @param str 字符串
   * @return 返回对应的Date对象。
   */
  ...
  /**
   * 获取字符串的日期格式。如果字符串不在[{@link #MONTH}/{@link #DAY}/ {@link #MINUTE} / {@link #SECOND}/
   * {@link #MILLISECOND} ]格式范围内将抛出异常。
   * 
   * @param str 字符串
   * @return 返回字符串的日期格式。
   */
