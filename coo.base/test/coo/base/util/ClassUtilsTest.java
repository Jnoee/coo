...
public class ClassUtilsTest {
  ...
  public void testFindClassesByParentClass() throws Exception {
    List<Class<?>> classes = ClassUtils.findClassesByParentClass(Object.class, "coo.base");
    log.debug("找到 " + classes.size() + " 个目标类：");
    for (Class<?> clazz : classes) {
      log.debug(clazz.getName());
    }
  ...
  public void testFindClassesByAnnotationClass() throws Exception {
    List<Class<?>> classes = ClassUtils.findClassesByAnnotationClass(Resource.class, "coo.base");
    log.debug("找到 " + classes.size() + " 个目标类：");
    for (Class<?> clazz : classes) {
      log.debug(clazz.getName());
    }
