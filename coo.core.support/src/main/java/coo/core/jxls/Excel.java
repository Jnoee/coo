...
public class Excel {
  /** Excel模版路径 */
  private static final String REPORT_DIR = "/META-INF/coo/report";
  ...
  public void init(String templateFileName, Map<String, ?> model) {
    try {
      InputStream in = getClass().getResourceAsStream(REPORT_DIR + "/" + templateFileName);
  ...
  public void init(ExcelData data) {
    try {
      ...
      InputStream in =
          getClass().getResourceAsStream(REPORT_DIR + "/" + data.getTemplateFileName());
      workbook = transformer.transformMultipleSheetsList(in, data.getSheetModels(), sheetNames,
          data.getSheetModelName(), data.getOtherModel(), data.getStartSheetNum());
      in.close();
    } catch (Exception e) {
      throw new UncheckedException("生成Excel时发生异常。", e);
    }
