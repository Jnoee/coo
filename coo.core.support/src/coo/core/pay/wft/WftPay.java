...
public class WftPay {
   ...
   * @return 返回响应对象。
   */
  public PayReply pay(PayData data) {
   ...
   * @param reply 响应对象
   */
  private void execute(WftPayQuery query, WftPayReply reply) {
    ...
    try {
      ...
      throw new UncheckedException("调用威富通支付接口发生异常。", e);
    }
  }
   ...
   * @param query 请求对象
   */
  private void processQuery(WftPayQuery query) {
  ...
  private void logError(String url, String queryXml, String replyXml, Exception e) {
    String msg = String.format("[威富通支付调用失败：%s]%n[请求消息：%s]%n[响应消息：%s]%n[异常信息：%s]", url, queryXml,
        replyXml, e.getMessage());
    log.error(msg);
  }

  public WftPayConfig getConfig() {
    return config;
  }

  public void setConfig(WftPayConfig config) {
    this.config = config;
  }
