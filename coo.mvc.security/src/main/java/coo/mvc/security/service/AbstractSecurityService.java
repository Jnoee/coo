package coo.mvc.security.service;
...
import coo.mvc.security.constants.AdminIds;
...
public abstract class AbstractSecurityService<O extends OrganEntity<O, U, A>, U extends UserEntity<U, A>, R extends RoleEntity<U, A>, A extends ActorEntity<O, U, R>> {
  ...
  @Transactional
  @AutoFillIn
  @DetailLog(target = "actor", code = "actor.edit.log", vars = {"actor.user.name", "actor.name"},
      type = LogType.ALL)
   ...
   * 
   * @param includePermissionCodes 包含权限编码
   ...
   * 
   * @param includePermissionCodes 包含权限编码
   * @param excludePermissionCodes 不包含权限编码
   * @return 返回满足权限条件的用户集合。
   */
  @Transactional(readOnly = true)
  @SuppressWarnings("unchecked")
