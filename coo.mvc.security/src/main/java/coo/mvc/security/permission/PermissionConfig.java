...
public class PermissionConfig {
  ...
  private static final String PERMISSION_PATH = "classpath*:" + PERMISSION_DIR + "*permissions.xml";
  ...
  private void initPermissions() throws IOException {
    List<Resource> permissionResources = SpringUtils.getResourcesByWildcard(PERMISSION_PATH);
    XmlMapper xmlMapper = new GenericXmlMapper();
    for (Resource permissionResource : permissionResources) {
      if (log.isDebugEnabled()) {
        log.debug("加载权限配置文件[{}]...", permissionResource.getURI().toString());
      }
      permissions
          .merge(xmlMapper.readValue(permissionResource.getInputStream(), Permissions.class));
    }
  }
  ...
  public List<Permission> getPermissions(String[] permissionCodes) {
    List<Permission> distPermissions = new ArrayList<>();
    if (!CollectionUtils.isEmpty(permissionCodes)) {
      for (String permissionCode : permissionCodes) {
        distPermissions.add(getPermission(permissionCode));
      }
    }
    return distPermissions;
  }
   ...
   * @return 返回权限ID列表对应的权限编码。
   */
  public BitCode getPermissionCode(List<Integer> permissionIds, Integer bitCodeLength) {
   ...
   */
  private void cleanEmptyPermissionGroup(List<PermissionGroup> permissionGroups) {
    Iterator<PermissionGroup> permissionGroupIterator = permissionGroups.iterator();
   ...
   * @param permission 权限
   */
  private void addPermissionToGroup(List<PermissionGroup> newPermissionGroups,
      Permission permission) {
    String permissionGroupName = findGroupByPermission(permission).getName();
  ...
  private PermissionGroup findGroupByPermission(Permission permissionToFind) {
    ...
    throw new UncheckedException("在所有权限分组中未找到权限编码为" + permissionToFind.getCode() + "的权限。");
  }
